...
function class(name, super)
    ...
    function cls.new()
        if super then
            ...
            setmetatable(cls, {__index = newSuper})
        end
...
function class(name, super)
    ...
    function cls.new()
        ...
        if instance.super then
            local newSuper = instance.super.new()
            setmetatable(instance, {__index = newSuper})
        end
...
function table.icontains(tbl, obj, func)
    if func then
...
function string.string2time(timeString)
...
function os.endDay()
    local expireTime = os.time(
                {year=os.date("%Y"),month=os.date("%m"),day=os.date("%d"),hour= 23, min=59, sec=59}
            ) - os.time()
